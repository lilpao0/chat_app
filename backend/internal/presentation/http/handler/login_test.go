...
func TestLoginHTTP(t *testing.T) {
	...
	for _, tc := range []struct {
		err    error
		status int
	}{{nil, 200}, {auth.ErrInvalidCredentials, 401}, {auth.ErrInvalidInput, 400}, {errors.New("PRIVATE ERROR"), 500}} {
		usecase := loginFunc(func(ctx context.Context, in auth.LoginInput) (auth.LoginResult, error) {
			...
			return auth.LoginResult{User: auth.PublicUser{ID: 1}, Token: auth.AccessToken{Value: "issued-token", ExpiresAt: time.Date(2026, 9, 22, 0, 0, 0, 0, time.UTC)}}, tc.err
		})
		r := gin.New()
		...
		if tc.status == 200 && (!strings.Contains(out.Body.String(), `"expires_at":"2026-09-22T00:00:00Z"`) || out.Header().Get("Cache-Control") != "no-store") {
